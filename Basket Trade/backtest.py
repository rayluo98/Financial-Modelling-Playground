...
class backtestEngine(object):
    ...
    @classmethod
    def reloadEngine(cls, btHist: pd.DataFrame):
        cls.bt_hist = btHist

    @classmethod
    def applySignal(cls, signal: pd.DataFrame, isMeanRevert: bool=False):
        ## removes existing signal
        if ("signal" in cls.bt_hist):
            cls.bt_hist.drop(cls.bt_cols, errors='ignore', inplace=True, axis=1)
        cls.bt_hist = cls.bt_hist.merge(signal, on ="Date", how='left')
    ...
    @classmethod
    def printResult(cls, isMeanRevert: bool=False):
        bdf = cls.bt_hist
